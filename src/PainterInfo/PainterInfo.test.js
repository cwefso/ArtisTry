...
describe('Painter Page', () => {
  ...
  it('should render a gallery', async () => {
    await waitFor(() => {
      usePaintings.mockResolvedValueOnce([
      {
  ...
  const { getByLabelText, getAllByTestId } = render(<MemoryRouter><PainterInfo info = {info}/></MemoryRouter>);
  const gallery = getByLabelText('gallery')
  const paintings = within(gallery).getAllByTestId('painting')
